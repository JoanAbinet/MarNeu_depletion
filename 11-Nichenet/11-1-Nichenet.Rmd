...
Capillary_cells <- subset(Capillary_cells, Condition %in% "PBS")
```
...
```{r}
# pdf("CP_Endo_Neutro_PBS.pdf", width = 8, height = 8) # measurement in inches 
...
# Expression signature of target genes in neutrophils
```{r, message=FALSE, warning=FALSE}
neutro_depletion <- AddModuleScore(neutro_depletion, features = list(c("Ccl3","Ccl4","Tnfsf14","Cxcr4", "Tnf", "Bhlhe40", "Icam1", "Ier3", "Nfkbia", "Sqstm1", "Trib1", "Ubc", "Il1rn", "Il1a", "Olr1", "Sod2", "Dusp2", "Lfng", "Mafk", "Mapkapk3")), name = "Target_gene_score")

neutro_depletion$condition <- neutro_depletion$orig.ident
neutro_depletion$condition <- as.factor(neutro_depletion$condition)
levels(neutro_depletion$condition) <- c("MarNeu treated", "MarNeu treated", "Control")

VlnPlot(neutro_depletion, features = "Target_gene_score1", group.by = "condition", pt.size = 0, cols = c("#00978a", "grey"))
# ggsave("scoring_targetGene_neutro.png", width = 9, height = 7)
